...
namespace Antmicro.Renode.Peripherals.GPIOPort
{
    public class QorIQ_GPIO : BaseGPIOPort, IDoubleWordPeripheral, IKnownSize, IEndiannessAware
    {
        ...
        public override void OnGPIO(int number, bool value)
        {
            ...
            lock(locker)
            {
                // The data associated to GPIO i is stored in the i-th space of the array
                // As opposed to register accesses where a write to GPIO 0 (LSB) should update the 1st space of the array
                var previousState = data[number];
                data[number] = value;
        ...
        private Dictionary<long, DoubleWordRegister> BuildRegisterMap()
        {
            var registersDictionary = new Dictionary<long, DoubleWordRegister>
            {
                ...
                {(long)Registers.Mask, new DoubleWordRegister(this)
                    ...
                    writeCallback: (id, _, val) => {
                            this.NoisyLog("Write to GPIMR : GPIO {0} interrupt set to {1}", NumberOfPins - 1 - id, val?"unmasked":"masked");
        ...
        private void UpdateIRQ()
        {
            ...
            this.DebugLog("Updating IRQ to {0}", flag);
            IRQ.Set(flag);
        }
        ...
        {
            this.NoisyLog("UpdateSingleInterruptRequest for line {0}, old value {1} -> new value {2}", i, oldState, currentState);
