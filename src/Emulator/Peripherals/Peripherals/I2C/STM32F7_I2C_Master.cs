...
// April 2026: The driver has been updated to ensure proper functionality
//             in master mode when an STM32H7xx workaround fixing
//             "Transmission stalled after first byte transfer" is applied 
//             by I2C driver in the FW (zephyr/drivers/i2c/i2c_ll_stm32_v2.c)
...
{
    public sealed class STM32F7_I2C_Master : SimpleContainer<II2CPeripheral>, II2CPeripheral, IDoubleWordPeripheral, IKnownSize
    {
        public STM32F7_I2C_Master(IMachine machine) : base(machine)
        {
        ...
        private void StartTransfer()
        {
            ...
            // Handle pending Tx data (if actual) once Transfer is started
            HandlePendingDataWrite();
        }
        ...
        private void PendTransmitDataWrite(uint DataByte)
        {
            isPendingTxData = true;
            pendingDataByte = DataByte;
        }

        private void HandlePendingDataWrite()
        {
            if (isPendingTxData)
            {
                // Transfer pending data byte
                MasterTransmitDataWrite(pendingDataByte);
                ResetPendingData();
            }
        }
        ...
        {
            // Clear pending data byte and pending flag
            isPendingTxData = false;
            pendingDataByte = 0;
        }
        ...
        private void HandleTransmitDataWrite(uint newValue)
        {
            ...
            else
            {
                ...
                // if the byte has been pushed for Tx before StartTransfer, it 
                // will be handled once transaction is initiated by master
                PendTransmitDataWrite(newValue);
            }
        }
        ...
        // The API below is not used as of this version of the driver
        // due to a fix for the FW I2C driver workaround that makes it hard to
        // support slave mode simultaneously with master mode
        ...
        // Required to store a pending Data byte pushed before start of Transfer
        private uint pendingDataByte;

        // Required to indicate a Data byte pushed before start of Transfer
        private bool isPendingTxData;
