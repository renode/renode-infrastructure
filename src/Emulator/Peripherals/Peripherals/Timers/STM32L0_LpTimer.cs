        ...
        public STM32L0_LpTimer(Machine machine, long frequency) : base(machine.ClockSource, frequency, limit: 0x1, direction: Direction.Ascending, enabled: false, eventEnabled: true)
        {
            ...
            var registersMap = new Dictionary<long, DoubleWordRegister>
            {
                 {(long)Registers.InterruptAndStatus, new DoubleWordRegister(this)
                    ...
                    .WithTag("Counter direction change down to up (UP)", 5, 1)
                    .WithTag("Counter direction change up to down (DOWN)", 6, 1)
                    .WithReservedBits(7, 25)
                  },

                  {(long)Registers.InterruptClear, new DoubleWordRegister(this)
                    .WithTag("Compare match clear flag (CMPMCF)", 0, 1)
                    ...
                    .WithTag("External trigger edge event clear flag (EXTTRIGCF)", 2, 1)
                    ...
                    .WithTag("Counter direction change down to up clear flag (UPCF)", 5, 1)
                    .WithTag("Counter direction change up to down clear flag (DOWNCF)", 6, 1)
                    .WithReservedBits(7, 25)
                },

                // Caution: The LPTIM_IER register must only be modified when the LPTIM is disabled(ENABLE bit reset to ‘0’)
                {(long)Registers.InterruptEnable, new DoubleWordRegister(this)
                    .WithTag("Compare match interrupt enable (CMPMIE)", 0, 1)
                    .WithFlag(1, out autoReloadMatchInterruptEnable, name: "Autoreload match interrupt enable (ARRMIE)")
                    .WithTag("External trigger edge event interrupt enable (EXTTRIGIE)", 2, 1)
                ...
                {(long)Registers.Configuration, new DoubleWordRegister(this)
                    .WithTag("Clock selector (CKSEL)", 0, 1)
                    ...
                    .WithTag("Timeout enable (TIMOUT)", 19, 1)
                    .WithTag("Waveform Shape (WAVE)", 20, 1)
                    .WithTag("Waveform shape polarity (WAVPOL)", 21, 1)
                    .WithTag("Registers update mode (PRELOAD)", 22, 1)
                    .WithTag("Counter mode enabled (COUNTMODE)", 23, 1)
                    .WithTag("Encoder mode enable (ENC)", 24, 1)
