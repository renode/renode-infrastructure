    ...
    public class OpenTitan_SpiDevice : BasicDoubleWordPeripheral, IBytePeripheral, ISPIPeripheral, IKnownSize
    {
        ...
        public override uint ReadDoubleWord(long offset)
        {
            if(offset >= (long)Registers.Buffer)
            {
                ...
                this.Log(LogLevel.Debug, $"Reading from sram memory ; value at addr {offset - 0x1000}");
                return underlyingSramMemory.ReadDoubleWord(offset - (long)Registers.Buffer);
            }
            else
            {
        ...
        private void DefineRegisters()
        {
            ...
            Registers.Control.Define(this, 0x80000010)
                ...
                .WithEnumField<DoubleWordRegister, DeviceMode>(4, 2, out mode,
                    writeCallback: (_, val) =>
                    {
                        ...
                        {
                            this.Log(LogLevel.Error, "{0} opearation mode is not implemented. Setting mode back to FwMode", val);
            ...
            Registers.Configuration.Define(this, 0x7f00)
                ...
                .WithTaggedFlag("tx_order", 2)
                .WithTaggedFlag("rx_order", 3)
                .WithTag("timer_v", 8, 8)
            ...
            Registers.FifoLevel.Define(this, 0x80)
                // Despite the misleading name this register holds the watermark levels - at least thats what the spec suggest.
            ...
            Registers.AsyncFifoLevel.Define(this)
                .WithValueField(0, 8, FieldMode.Read, valueProviderCallback: (_) => { return rxFifo.CurrentDepth; }, name: "rxlvl")
                .WithValueField(16, 8, FieldMode.Read, valueProviderCallback: (_) => { return txFifo.CurrentDepth; }, name: "txlvl")
            ...
            Registers.ReceiverFifoSramPointers.Define(this)
                ...
                .WithValueField(16, 16, FieldMode.Read, valueProviderCallback: _ => rxFifo.WritePointer, name: "WPTR");

            Registers.TransmitterFifoSramPointers.Define(this)
                .WithValueField(0, 16, FieldMode.Read, valueProviderCallback: _ => txFifo.ReadPointer, name: "RPTR")
                .WithValueField(16, 16, valueProviderCallback: _ => txFifo.WritePointer, changeCallback: (_, val) =>
                    {
            ...
            Registers.ReceiverFifoSramAddresses.Define(this, 0x1fc0000)
                ...
                .WithWriteCallback((_, __) =>
                    {
                        if(!rxFifo.TryUpdateParameters(rxFifoBase.Value, rxFifoLimit.Value))
                        {
            ...
            Registers.TransmitterFifoSramAddresses.Define(this, 0x3fc0200)
                ...
                .WithWriteCallback((_, __) =>
                    {
                        if(!txFifo.TryUpdateParameters(txFifoBase.Value, txFifoLimit.Value))
                        {
                            this.Log(LogLevel.Error, "Parameters were rejected as an invalid range");
                        }
                    });
        ...
        private class SRAMCircularFifoRange
        {
            public SRAMCircularFifoRange(uint baseOffset, uint limit, ArrayMemory underlyingMemory)
            {
                ...
                UpdateParameters((ushort)baseOffset, (ushort)limit);
            }

            public bool TryUpdateParameters(uint baseOffset, uint limitOffset)
            {
                // Valid fifo addresses must fix in certain address space
                // Values that does not fit into this should not be used
                uint validSramAddressMask = ~(1u << 10);

                var addressesInvalid = ((baseOffset & validSramAddressMask) != 0) || ((limitOffset & validSramAddressMask) != 0);
                var addressesUnordered = baseOffset < limitOffset; 

                if(addressesInvalid || addressesUnordered)
                {
                    return false;
                }
            ...
            {
                if(!RangeFitsIntoMemory(baseOffset, limitOffset))
                {
                    // this is possible only through the misconfiguration of the initial parameters
                    throw new ArgumentException("Wrong parameters. The resulting buffer does not fit into the underlying memory");
                }
            ...
            public void ResetPointers()
            {
                ...
                this.status = FifoStatus.Empty;
            }

            public bool TrySetWritePointer(uint newValue)
            {
                if(PointerInDefinedRange(newValue))
                {
                    this.writePointer = checked((ushort)newValue);
                    return true;
                }
                return false;
            }
            ...
            public bool TrySetReadPointer(uint newValue)
            {
                if(PointerInDefinedRange(newValue))
                {
                    this.readPointer = checked((ushort)newValue);
                    return true;
                }
                return false;
            }
            ...
            public uint ReadPointer => readPointer | (uint)((readPhase ? 1 : 0) << 11);
            public uint WritePointer => writePointer | (uint)((writePhase ? 1 : 0) << 11);
            ...
            private bool RangeFitsIntoMemory(uint baseOffset, uint limitOffset)
            {
                return (baseOffset < underlyingMemory.Size) && (limitOffset < underlyingMemory.Size);
            }
