...
namespace Antmicro.Renode.Peripherals.SPI
{
    public class LS1043SPI : SimpleContainer<ISPIPeripheral>, IKnownSize, IDoubleWordPeripheral, IWordPeripheral, IProvidesRegisterCollection<DoubleWordRegisterCollection>, IProvidesRegisterCollection<WordRegisterCollection>, IEndiannessAware
    {
        ...
        {
            this.NoisyLog("WriteDoubleWord(): offset 0x{0}, value 0x{1}", offset.ToString("X8"), value.ToString("X8"));
            ...
            if((!Running || GetTxCount() >= TxFifoSize) && ((WRegisters)offset == WRegisters.SPI_CMD_PUSHR || (WRegisters)offset == WRegisters.SPI_DATA_PUSHR))
            { //As per doc
        ...
        public void WriteWord(long offset, ushort value)
        {
            ...
            if((!Running || GetTxCount() >= TxFifoSize))
            {
        ...
        private void DefineRegisters()
        {
            ...
            DWRegisters.SPI_MCR.Define(this as IProvidesRegisterCollection<DoubleWordRegisterCollection>, 0x00000001)
                ...
                }, writeCallback: (old, val) =>
                {
                    if(old != val) ClearTxFifo(); // Must reset counters between mode change
                })
                ...
                {
                    if(old != val) ClearRxFifo(); // Must reset counters between mode change
                })
                .WithFlag(11, name: "CLR_TXF - Clear TX FIFO", writeCallback: (old, written) =>
                ...
                .WithFlag(10, name: "CLR_RXF - Clear RX FIFO", writeCallback: (old, written) =>
                { //Always read as 0 - Writing 1 clears the counter (= callback) but no write is actually done
                    if(written)
                    {
                        ClearRxFifo();
            ...
            DWRegisters.SPI_SR.Define(this as IProvidesRegisterCollection<DoubleWordRegisterCollection>, 0x02010000)
                ...
                .WithFlag(28, out endOfQueueSR, FieldMode.Read | FieldMode.WriteOneToClear, name: "EOQF - End Of Queue Flag") // Set by hardware at the end of a command with EOQ set
                .WithReservedBits(26, 2)
                .WithFlag(25, out txFifoNotFull, FieldMode.Read | FieldMode.WriteOneToClear, name: "TFFF - Transmit Fifo Fill Flag", changeCallback: (old, newer) =>
                {
                ...
                .WithFlag(24, FieldMode.Read, name: "BSYF - Busy Flag", valueProviderCallback: (_) => (cmdHowMuchLeft > 1)) //Set when cyclic command underway
                ...
                {
                    this.Log(LogLevel.Noisy, "Receive Fifo Drain flag is set to {0}", RxDrainFlag ? "not empty" : "empty");
                    return RxDrainFlag;
                })
                .WithFlag(16, out cmdFifoNotFull, FieldMode.Read | FieldMode.WriteOneToClear, name: "CMDFFF - Command FIFO Fill Flag", changeCallback: (old, newer) =>
                {
            ...
            DWRegisters.SPI_RSER.Define(this as IProvidesRegisterCollection<DoubleWordRegisterCollection>, 0x0)
                .WithFlag(31, out transferCompleteInterrupt, name: "TCF_RE - Transmission Complete Request Enable", changeCallback: (old, newer) =>
                {
                    ...
                    if(newer) UpdateInterrupts(); // I want an interrupt to be raised if flag is up and interrupt is unmasked ; but not be lowered if masked
                }, readCallback: (_, newer) =>
                {
                ...
                .WithReservedBits(0, 15); //Any modification on the RSER register should trigger interrupt updates
            ...
            cmd_pushr_reg = WRegisters.SPI_CMD_PUSHR.Define(this as IProvidesRegisterCollection<WordRegisterCollection>, 0x0) //As seen in Peripherals/I2C/SAMD21_I2C.c
                ...
                .WithWriteCallback((old, cmd) => // Writing the whole register in the CMD FIFO
                {
                    if(txDisabled.Value)
                    {
                        SetNewTxCount(1); // Filled PUSHR so at least 1 data ; no FIFO so at max 1 data
                        txFifoNotFull.Value = true;
            ...
            data_pushr_reg = WRegisters.SPI_DATA_PUSHR.Define(this as IProvidesRegisterCollection<WordRegisterCollection>, 0x0) //As seen in Peripherals/I2C/SAMD21_I2C.c
                ...
                .WithWriteCallback((old, data) =>
                {
                    ...
                    {
                        SetNewTxCount(1); // Filled PUSHR so at least 1 data ; no FIFO so at max 1 data
                        txFifoNotFull.Value = false; // When there is no FIFO, only 1 spot can be filled
                        this.Log(LogLevel.Noisy, "Registered data : {0} ; FIFO disabled", data.ToString("X8"));
                        while(inLaunchTransfer) ;
                        if(!LaunchTransfer()) // Does a transfer as soon as there is data (not only command) to be transferred
                        {
                    ...
                    {
                        if(GetTxCount() >= TxFifoSize)
                        { //Ignores attempt to push data to a full TX FIFO
                            data_pushr_reg.Value = old; //Restores the old value
                            this.Log(LogLevel.Warning, "Could not push data : {0} to TX FIFO because it is full. PUSHR register restored to its old state", data.ToString("X8"));
                        }
                        else
                        {
                            bool couldPush = PushDataToTXFIFO(data); // Actually pushes the data in the correct register and updates pointers
                            if(couldPush)
                            {
                                this.Log(LogLevel.Noisy, "Pushed and sent data : {0} to TX FIFO", data.ToString("X8"));
                            ...
                            {
                                this.Log(LogLevel.Warning, "Could not push data : {0} to TX FIFO", data.ToString("X8"));
                            }
                        }
                    }
                });
            ...
            popr_reg = DWRegisters.SPI_POPR.Define(this as IProvidesRegisterCollection<DoubleWordRegisterCollection>, 0x0)
                ...
                }, valueProviderCallback: old =>
                {
                    ...
                    {
                        SetNewRxCount(0); // Value should not be more than 1 since fifo is disabled ; bringing this value down to 0
                        UpdateInterrupts();
            ...
            DWRegisters.SPI_SREX.Define(this as IProvidesRegisterCollection<DoubleWordRegisterCollection>, 0x0)
                ...
                .WithFlag(14, out xtendedTxCounter, FieldMode.Read, name: "TXCTR4 - TX FIFO Counter Extension")
                .WithReservedBits(12, 2)
                .WithFlag(11, out xtendedRxCounter, FieldMode.Read, name: "RXCTR4 - RX FIFO Counter Extension")
                .WithReservedBits(9, 2)
                .WithValueField(4, 5, out cmdCounter, name: "CMDCTR - CMD FIFO Counter")
                .WithValueField(0, 4, out cmdNext, name: "CMDNXTPTR - Command Next Pointer");
        }
        ...
        private ulong GetTxCount()
        {
            ulong val = txCounter.Value;
            val += (xtendedTxCounter.Value ? (ulong)16 : (ulong)0);
            return val;
        }

        private void SetNewTxCount(ulong value)
        {
            txCounter.Value = value % 16;
            xtendedTxCounter.Value = (value / 16) == 1;
        }

        private ulong GetRxCount()
        {
            ulong val = rxCounter.Value;
            val += (xtendedRxCounter.Value ? (ulong)16 : (ulong)0);
            return val;
        }

        private void SetNewRxCount(ulong value)
        {
            rxCounter.Value = value % 16;
            xtendedRxCounter.Value = (value / 16) == 1;
        }
        ...
        private void UpdateInterrupts()
        {
            ...
            flag |= cmdFifoNotFull.Value && cmdFifoInterrupt.Value;
            flag |= endOfQueueSR.Value && endOfQueueSRInterrupt.Value;
            flag |= txFifoNotFull.Value && txFifoNotFullInterrupt.Value;
        ...
        private void PushDataToCMDFIFO(ulong data)
        {
            ...
            cmdFifoNotFull.Value = cmdCounter.Value < CmdFifoSize;
            UpdateInterrupts();
        }

        private bool PushDataToTXFIFO(ulong data)
        {
            this.NoisyLog("In PushDataTXFIFO()");
            if(cmdCounter.Value == 0 && cmdHowMuchLeft == 0)
            {// No data in cmd fifo so this entry is invalid
            ...
            {
                // Fifo Full case handled inside the write handler of the push register
                uint nextToWrite = (uint)(txNext.Value + GetTxCount()) % TxFifoSize;

                txdata[nextToWrite].Value = data;
                this.NoisyLog("Data has been pushed to TXFIFO {0}", nextToWrite);

                SetNewTxCount(GetTxCount() + 1);
                txFifoNotFull.Value = GetTxCount() < TxFifoSize;
            }
            UpdateInterrupts();
            return true;
        }
        ...
        {
            this.NoisyLog("In SendDecypher() with value 0x{0}", cmd.ToString("x4")); cmdContinuousPSCEnable = (((cmd >> 15) & 0x1) == 1); // Bit 15
        ...
        private void EnqueueResponse(uint receivedWord)
        {
            if(rxDisabled.Value)
            {
                if(GetRxCount() > 0) //This is an overflow
                {
                    rxFifoOverflow.Value = true;
                    if(rxFifoOverwriteOnOverflow.Value)
                    {
                        popr_reg.Value = receivedWord;
                        this.NoisyLog("POPR register already contains data ; data was overwritten");
                    }
                    else // else data is discarded, we do nothing
                        this.NoisyLog("POPR register already contains data ; data was discarded");
                }
                else // Store value and increment rxctr
                {
                    popr_reg.Value = receivedWord;
                    SetNewRxCount(GetRxCount() + 1);
                    this.NoisyLog("POPR register was filled with data {0}", receivedWord);
                }
            }
            else //RX FIFO is enabled
            {
                ulong rxFifoCount = GetRxCount();
                ulong nextRxSpace =(rxNext.Value + rxFifoCount) % RxFifoSize;
                if(rxFifoCount >= RxFifoSize) //We overflowed
                {
                    nextRxSpace = (rxNext.Value - 1) % RxFifoSize;
                    rxFifoOverflow.Value = true;
                    if(rxFifoOverwriteOnOverflow.Value)
                    {
                        rxdata[nextRxSpace].Value = receivedWord;
                        this.NoisyLog("RX FIFO is full ; data was overwritten in spot {0}", nextRxSpace);
                    }
                    else // else data is discarded, we do nothing
                        this.NoisyLog("RX FIFO is full ; data was discarded");
                }
                else // Store value and increment rxctr
                {
                    rxdata[nextRxSpace].Value = receivedWord;
                    SetNewRxCount(rxFifoCount + 1);
                    this.NoisyLog("RX FIFO {0} register was filled with data {1}", nextRxSpace, receivedWord);
                }
        ...
        private bool TryExchange()
        {
            ...
            else
            {
                ...
                if(IsFrame32Bits() && GetTxCount() < 2) //Using the command we prepared earlier
                {
            ...
            else
            {
                ...
                else
                {
                    data = data_pushr_reg.Value;
                    SetNewTxCount(0);
                    txFifoNotFull.Value = true; // 0 < 1, so Fifo is not full
                }
        ...
        private bool RxDrainFlag => GetRxCount() > 0;

        private static uint TxFifoSize => 16;

        private static uint CmdFifoSize => 16;

        private static uint RxFifoSize => 16;
        ...
        private IFlagRegisterField txFifoNotFull;
        ...
        private IFlagRegisterField cmdFifoNotFull;
        ...
        // SPI_SREX Flags
        private IFlagRegisterField xtendedTxCounter;
        private IFlagRegisterField xtendedRxCounter;
