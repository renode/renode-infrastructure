    ...
    public partial class Cadence_xSPI
    {
        ...
        private void CommonSequenceDiscovery(SFDPData sfdp, bool fullDiscovery)
        {
            ...
            if(fullDiscovery)
            {
                ...
                pageSizeProgram.Value = (ulong)jedecParam.PageSize;
        ...
        private void ProgramSequenceDiscovery(SFDPData sfdp, bool fullDiscovery)
        {
            ...
            if(fullDiscovery)
            {
                var programSequencePriority = new List<SequenceConfiguration>  {
                    ...
                    new SequenceConfiguration(true, 1, 1, 1, (ulong)Math.Pow(2, discoveryABNUM.Value ? 4 : 3), 0, defaultProgramCmd)
                };

                var foundProgram = programSequencePriority.First(v=>v.IsSupported);

                programCmd.Value = foundProgram.Cmd;
                programAddressCount.Value = foundProgram.AddressCount;
                programCmdIOS.Value = foundProgram.CmdIOS;
                programAddressIOS.Value = foundProgram.AddressIOS;
                programDataIOS.Value = foundProgram.DataIOS;
                programDummyCount.Value = foundProgram.DummyBytes;
            }
            else
            {
                programCmd.Value = discoveryABNUM.Value ? defaultProgram4ByteCmd : defaultProgramCmd;
                programAddressCount.Value = discoveryNumberOfLines.Value;
                programCmdIOS.Value = 0;
                programAddressIOS.Value = 0;
                programDataIOS.Value = 0;
                programDummyCount.Value = 0UL;
            }
        ...
        private void ReadSequenceDiscovery(SFDPData sfdp, bool fullDiscovery)
        {
            var support4ByteParam = sfdp?.Support4ByteCommandsParameter;
            var jedecParam = sfdp.JedecParameter;
            var defaultReadSequence = new SequenceConfiguration(true, 1, 1, 2, discoveryNumberOfLines.Value, 8, 0xB);
            ...
            if(fullDiscovery)
            {
                ...
                case 0:
                    ...
                    {
                            new SequenceConfiguration(jedecParam.SupportsRead1s8s8s(out var command188), 1, 8, 8, discoveryNumberOfLines.Value, 0, command188),
                            new SequenceConfiguration(jedecParam.SupportsRead1s1s8s(out var command118), 1, 1, 8, discoveryNumberOfLines.Value, 0, command118),
                            new SequenceConfiguration(jedecParam.SupportsRead1s4s4s(out var command144), 1, 4, 4, discoveryNumberOfLines.Value, 0, command144),
                            new SequenceConfiguration(jedecParam.SupportsRead1s1s4s(out var command114), 1, 1, 4, discoveryNumberOfLines.Value, 0, command114),
                            new SequenceConfiguration(jedecParam.SupportsRead1s2s2s(out var command122), 1, 2, 2, discoveryNumberOfLines.Value, 0, command122),
                            new SequenceConfiguration(jedecParam.SupportsRead1s1s2s(out var command112), 1, 1, 2, discoveryNumberOfLines.Value, 0, command112),
                    };
                ...
                readCmdIOS.Value = foundReadSequence.CmdIOS;
                readAddressIOS.Value = foundReadSequence.DataIOS;
                readDataIOS.Value = foundReadSequence.DataIOS;
                readDummyCount.Value = foundReadSequence.DummyBytes;
                readAddressCount.Value = foundReadSequence.AddressCount;
            }
            else
            {
                switch(discoveryNumberOfLines.Value)
                ...
                case 1:
                    ...
                    readDummyCount.Value = (byte)(discoveryDummyCount.Value ? 8 : 0);
                    readCmd.Value = (byte)commandMap[Convert.ToInt32(discoveryABNUM.Value), Convert.ToInt32(discoveryDummyCount.Value)];
                    readCmdIOS.Value = 1;
                    readAddressIOS.Value = 1;
                    readDataIOS.Value = 1;
                    break;
                case 2:
                    readDummyCount.Value = (byte)(discoveryDummyCount.Value ? 10 : 8);
                    readCmd.Value = discoveryABNUM.Value ? 0xCUL : 0xBUL;
                    readCmdIOS.Value = 2;
                    readAddressIOS.Value = 2;
                    readDataIOS.Value = 2;
                    break;
                case 4:
                    readDummyCount.Value = (byte)(discoveryDummyCount.Value ? 10 : 8);
