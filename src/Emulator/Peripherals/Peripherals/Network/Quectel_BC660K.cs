...
namespace Antmicro.Renode.Peripherals.Network
{
    public class Quectel_BC660K : AtCommandModem, IGPIOReceiver, INumberedGPIOOutput
    {
        ...
        {
            // TODO: the real modem sends these, so we want to too, but this should be triggered
            // by something else than AT&W
            var d = 200UL;
            ExecuteWithDelay(() => SendString("+CSCON: 1"), d += 2000); // stub, CSCON: 1 -> connected
            ExecuteWithDelay(() => SendString(CeregContent(true)), d += 100);
            ExecuteWithDelay(() => SendString($"+IP: {NetworkIp}"), d += 1000); // stub, IP URC means successfully registered
            return Ok; // stub
        }
