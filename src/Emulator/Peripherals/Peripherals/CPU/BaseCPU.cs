        ...
        protected virtual void InnerPause(bool onCpuThread, bool checkPauseGuard)
        {
            ...
            if(onCpuThread)
            {
                ...
                do
                {
                    ...
                    if(!success)
                    {
                        /// We weren't able to interrupt <see cref="TimeHandle"/>.
                        /// Check marker to distinguish an usual timeout from a deadlock.
                        if(pauseLockTimeHandleMarker)
                        {
                            /// We have a deadlock with other thread in <see cref="StartCPUThreadTimeHandle"/>.
                            /// Release <see cref="pauseLock"/> and try <see cref="TimeHandle.Interrupt"/> again later after deadlock is resolved.
                            /// In rare cases we expect deadlock as a result of starting and pausing cpu from different threads.
                        ...
                        {
                            /// We have an usual timeout. Try <see cref="TimeHandle.Interrupt"/> again.
                            /// Leave a warning here, as this may require special attention performance-wise.
                            this.Log(LogLevel.Warning, "Trying to stop CPU execution, but it takes longer than expected...");
                        }
                    }
                }
                while(!success);
        ...
        private void StartCPUThreadTimeHandle()
        {
            ...
            lock(pauseLock)
            {
                if(!pauseLockTimeHandleMarker)
                {
                    ...
                    /// We allow to win pause in such case because pausing cpu is more "explicit" operation
