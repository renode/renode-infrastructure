...
{
    public class GPIO_3 : BaseGPIOPort, IDoubleWordPeripheral, IKnownSize
    {
        public GPIO_3(Machine machine, bool logRegisterAccess = false) : base(machine, NumberOfPins * NumberOfPorts)
        {
            ...
            switch(signalSource)
            {
                case SignalSource.USART0:
                {
                    switch(signalType)
                    {
                        ...
                        default:
                            throw new Exception(string.Format("GPIO Signal type {0} for USART0 not supported", signalType));
                    }
                    break;
                }
                case SignalSource.EUSART0:
                {
                    switch(signalType)
                    {
                        ...
                        default:
                            throw new Exception(string.Format("GPIO Signal type {0} for EUSART0 not supported", signalType));
                    }
                    break;
                }
                case SignalSource.EUSART1:
                {
                    switch(signalType)
                    {
                        ...
                        default:
                            throw new Exception(string.Format("GPIO Signal type {0} for EUSART1 not supported", signalType));
                    }
                    break;
                }
                default:
                    throw new Exception(string.Format("GPIO Signal source {0} not supported", signalSource));
            }
