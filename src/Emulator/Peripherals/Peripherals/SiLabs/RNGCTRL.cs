...
namespace Antmicro.Renode.Peripherals.Silabs
{
    ...
    public class RngCtrl : IBusPeripheral
    {
        public RngCtrl(Machine machine, bool logRegisterAccess = false, bool logInterrupts = false)
        {
            this.machine = machine;
            this.LogRegisterAccess = true;
...
#region methods
        ...
        private uint GetKeyDoubleWord(uint index)
        {
            if (index > 3)
            {
                throw new Exception("GetKeyDoubleWord(): index invalid");
            }
        ...
        private void SetKeyDoubleWord(uint index, uint value)
        {
            if (index > 3)
            {
                throw new Exception("SetKeyDoubleWord(): index invalid");
            }
