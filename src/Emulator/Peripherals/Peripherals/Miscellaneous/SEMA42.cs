...
namespace Antmicro.Renode.Peripherals.Miscellaneous
{
    ...
    {
        public SEMA42(IMachine machine, NXP_XRDC xrdc = null)
        {
            ...
            Registers.Gate.DefineMany(registers, NrOfGates, (register, offset) =>
            {
                ...
                register
                    .WithValueField(0, 4, out gateState[gateIdx],
                        ...
                        {
                            if(xrdc != null && sysbus.TryGetTransactionInitiator(out var initiator))
                            {
                                var domain = xrdc.GetDomainId(initiator);
            ...
            Registers.ResetGate.Define(registers)
                ...
                .WithValueField(8, 4, FieldMode.Read,
                    valueProviderCallback: _ =>
                    {
                        if(xrdc != null && sysbus.TryGetTransactionInitiator(out var initiator))
                        {
                            var did = xrdc.GetDomainId(initiator);
                            if(did.HasValue)
                            {
                                return did.Value;
                            }
