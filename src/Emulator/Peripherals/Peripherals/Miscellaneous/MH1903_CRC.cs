...
namespace Antmicro.Renode.Peripherals.Miscellaneous
{
    public class MH1903_CRC : BasicDoubleWordPeripheral, IBytePeripheral, IKnownSize
    {
        ...
        {
            // CRC_DATA register (offset 0x08) accepts byte writes for feeding data
            if(offset == (long)Registers.CRC_DATA)
            {
                ProcessByte(value);
                return;
            }
        ...
        {
            Registers.CRC_CSR.Define(this, resetValue: 0x00000000)
                .WithFlag(0, name: "POLY_SEL",
                    writeCallback: (_, value) => polynomialSelect = value,
                    valueProviderCallback: _ => polynomialSelect)
                .WithFlag(1, name: "TYPE_SEL",
            ...
            // CRC_DATA register at 0x08
            // Write (byte): DIN - Feed data byte into CRC
            // Read (32-bit): DOUT - Read CRC result
            RegistersCollection.DefineRegister((long)Registers.CRC_DATA)
                .WithValueField(0, 32,
                    writeCallback: (_, value) =>
                    {
