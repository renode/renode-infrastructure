...
namespace Antmicro.Renode.Peripherals.Miscellaneous.SiLabs
{
    public class EFR32_GPCRC : IDoubleWordPeripheral, IKnownSize
    {
        public EFR32_GPCRC()
        {
            var registerMap = new Dictionary<long, DoubleWordRegister>
            {
                ...
                {(long)Registers.InputData, new DoubleWordRegister(this)
                    .WithValueField(0, 32, FieldMode.Write, writeCallback: (_, value) => CalculateCrc32(value), name: "INPUTDATA")
                },
        ...
        private void CalculateCrc32(uint data)
        {
            if(isEnabled)
            {
                foreach(var b in BitConverter.GetBytes(data))
                {
                    crc.Value = crc32Tab[(crc.Value ^ b) & 0xff] ^ (crc.Value >> 8);
                }
            }
        }
        ...
        //Crc32 table taken from binascii python module https://github.com/python/cpython/blob/master/Modules/binascii.c#L1026
        // or we can use this specific function to generate it https://github.com/Michaelangel007/crc32/blob/master/src/crc32.h#L113
        private static uint [] crc32Tab = new uint[256] {
