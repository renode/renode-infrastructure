...
#region fields
        ...
        private SiLabs_IKeyStorage ksuStorage;
...
#region command handlers
        private ResponseCode HandleImportKeyCommand(uint inputDma, uint outputDma, uint[] commandParams, uint commandParamsCount)
        {
            ...
            {
                parent.Log(LogLevel.Noisy, "Storing key in KSU at index {0}", keyIndex);
        ...
        private ResponseCode HandleTransferKeyCommand(uint inputDma, uint outputDma, uint[] commandParams, uint commandParamsCount)
        {
            ...
            KeyMode updateMode = (KeyMode)((keyUpdateMetadata >> 8) & 0x3);
            uint updateIndex = keyUpdateMetadata & 0xFF;

            parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Key Metadata: keyIndex={0} keyType={1} keyMode={2} keyRestriction={3} updateMode={4} updateIndex={5}",
                     keyIndex, keyType, keyMode, keyRestriction, updateMode, updateIndex);
            ...
            // Perform the key transfer based on the specified operations
            if (keyMode == KeyMode.Wrapped)
            {
                if (updateMode == KeyMode.Volatile)
                {
                    // wrapped_RAM → SE_VOLATILE
                    parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Moving wrapped key from RAM to SE_VOLATILE");
                    volatileKeys[updateIndex] = inputKeyData;
                }
                else if (updateMode == KeyMode.Wrapped)
                {
                    // wrapped_RAM → wrapped_RAM (with different authdata)
                    parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Moving wrapped key from RAM to wrapped_RAM with different authdata");
                    // For simplicity, we assume the new auth data is appended to the key data
                    byte[] updatedKeyData = new byte[inputKeyData.Length + newAuthData.Length];
                    Array.Copy(inputKeyData, 0, updatedKeyData, 0, inputKeyData.Length);
                    Array.Copy(newAuthData, 0, updatedKeyData, inputKeyData.Length, newAuthData.Length);
                    volatileKeys[updateIndex] = updatedKeyData;
                }
                else
                {
                    return ResponseCode.InvalidParameter;
            ...
            else if (keyMode == KeyMode.Volatile)
            {
                ...
                byte[] key = volatileKeys[keyIndex];
                volatileKeys.Remove(keyIndex);
                parent.Log(LogLevel.Noisy, "Transferring volatile key=[{0}]", BitConverter.ToString(key));

                if (updateMode == KeyMode.Wrapped)
                {
                    // SE_VOLATILE → wrapped_RAM
                    parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Moving key from SE_VOLATILE ({0}) to wrapped_RAM ({1})", keyIndex, updateIndex);
                    volatileKeys[updateIndex] = key;
                }
                else if (updateMode == KeyMode.Volatile)
                {
                    // SE_VOLATILE → SE_VOLATILE
                    parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Moving key from SE_VOLATILE to SE_VOLATILE");
                    volatileKeys[updateIndex] = key;
                }
                else if (updateMode == KeyMode.Ksu)
                {
                    // SE_VOLATILE → KSU_VOLATILE
                    parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Moving key from SE_VOLATILE ({0}) to KSU_VOLATILE ({1})", keyIndex, updateIndex);
                    ksuStorage.AddKey(updateIndex, key);
                }
                else
                {
                    return ResponseCode.InvalidParameter;
            ...
            else if (keyMode == KeyMode.Ksu)
            {
                ...
                byte[] key = ksuStorage.GetKey(keyIndex);
                ksuStorage.RemoveKey(keyIndex);
                parent.Log(LogLevel.Noisy, "Transferring KSU key=[{0}]", BitConverter.ToString(key));

                if (updateMode == KeyMode.Volatile)
                {
                    // KSU_VOLATILE → SE_VOLATILE
                    parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Moving key from KSU_VOLATILE to SE_VOLATILE");
                    volatileKeys[updateIndex] = key;
                }
                else if (updateMode == KeyMode.Ksu)
                {
                    // KSU_VOLATILE → KSU_VOLATILE
                    parent.Log(LogLevel.Noisy, "TRANSFER_KEY: Moving key from KSU_VOLATILE to KSU_VOLATILE");
                    ksuStorage.AddKey(updateIndex, key);
                }
                else
                {
            ...
            // If keyMetadata.key_mode is VOLATILE, no data will be returned
            if (keyMode == KeyMode.Volatile || keyMode == KeyMode.Ksu)
            {
                return ResponseCode.Ok;
            }
            ...
            WriteToRam(volatileKeys[updateIndex], 0, outputKeyDataPtr, outputKeyDataLength);
            parent.Log(LogLevel.Noisy, "TRANSFER_KEY: OutputKeyData=[{0}]", BitConverter.ToString(volatileKeys[updateIndex]));
