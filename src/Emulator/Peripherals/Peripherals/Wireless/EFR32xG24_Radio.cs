...
{
    public class EFR32xG24_Radio : IBusPeripheral, IRadio
    {
        public EFR32xG24_Radio(Machine machine, MappedMemory ram, CortexM sequencer)
        {
            ...
            proTimer = new LimitTimer(machine.ClockSource, HfxoFrequency, this, "protimer", initialTimerLimit, direction: Direction.Ascending,
                                      enabled: false, eventEnabled: true, autoUpdate: true);
            ...
            txTimer = new LimitTimer(machine.ClockSource, MicrosecondFrequency, this, "txtimer", initialTimerLimit, direction: Direction.Ascending,
                                             enabled: false, workMode: WorkMode.OneShot, eventEnabled: true, autoUpdate: true);
            txTimer.LimitReached += RAC_TxTimerLimitReached;

            rxTimer = new LimitTimer(machine.ClockSource, MicrosecondFrequency, this, "rxtimer", initialTimerLimit, direction: Direction.Ascending,
                                             enabled: false, workMode: WorkMode.OneShot, eventEnabled: true, autoUpdate: true);
            ...
            for(var idx = 0; idx < PROTIMER_NumberOfCaptureCompareChannels; ++idx)
            {
                ...
                if (!PROTIMER_LightWeightMode)
                {
                    PROTIMER_captureCompareChannel[i].timer = new LimitTimer(machine.ClockSource, HfxoFrequency, this, $"protimer-cc{i}", initialTimerLimit, Direction.Ascending,
                                                                            enabled: false, workMode: WorkMode.OneShot, eventEnabled: true, autoUpdate: true);
                    PROTIMER_captureCompareChannel[i].timer.LimitReached += delegate
                    {
            ...
            // RAC stuff
            RAC_seqStorage = new IValueRegisterField[RAC_NumberOfSequencerStorageRegisters];
        ...
        public void Reset()
        {
            ...
            if (!PROTIMER_LightWeightMode)
            {
                ...
                {
                    PROTIMER_captureCompareChannel[i].timer.Reset();
                }
            }
        ...
        public void ReceiveFrame(byte[] frame)
        {
            ...
            var foundSyncWord = FRC_CheckSyncWords(frame);

            if(RAC_currentRadioState != RAC_RadioState.RxFrame)
            {
                this.Log(LogLevel.Info, "Dropping: {0}", BitConverter.ToString(frame));
                UpdateInterrupts();
                return;
            }
            
            this.Log(LogLevel.Info, "Received at {0}: {1}", GetTime(), BitConverter.ToString(frame));

            // TODO: for now we hard-code the RSSI of every packet
            AGC_FrameRssiIntegerPart = AGC_FrameRssiIntegerPartHardcodedValue;
            
            if (FRC_DisassembleFrame(frame, foundSyncWord ? MODEM_SyncWordBytes : 0))
            {
                // TODO: We delay the "RX completion" a bit (50us) to give the sequencer time to perform 
                // filtering and other packet inspection that happens while in RxFrame. Without this delay
                // packets are passed up to the lower MAC with a non-success status that causes an assert.
                // Needs to be further investigated, early debugging points to potential issue with the
                // ScheduleRx call issued at init time by the lower MAC that appears to not "finish" properly.
                rxTimer.Limit = 50;
        ...
        {
            RAC_TxEnable = false;
            RAC_txAfterFramePending.Value = false;
            ...
            this.Log(LogLevel.Info, "Sending frame at {0}: {1}", GetTime(), Misc.PrettyPrintCollectionHex(frame));
...
#region Region Accessors
        ...
        public uint ReadDoubleWordFromHostMailboxNonSecure(long offset)
        {
            ...
            return Read<HostMailboxRegisters>(hostMailboxRegistersCollection, "Host Mailbox (RFMAILBOX_NS)", offset);
        }
        ...
        public byte ReadByteFromHostMailboxNonSecure(long offset)
        {
            ...
            return ReadByte<HostMailboxRegisters>(hostMailboxRegistersCollection, "Host Mailbox (RFMAILBOX_NS)", offset);
        }

        [ConnectionRegionAttribute("hostmailbox_ns")]
        public void WriteDoubleWordToHostMailboxNonSecure(long offset, uint value)
        {   
            Write<HostMailboxRegisters>(hostMailboxRegistersCollection, "Radio Mailbox (RFMAILBOX_NS)", offset, value);
        }
...
#region Build Register Collections
        ...
        private DoubleWordRegisterCollection BuildBufferControllerRegistersCollection()
        {
            ...
            for(var index = 0; index < BUFC_NumberOfBuffers; index++)
            {
                ...
                registerDictionary.Add(startOffset + blockSize*i + rData32Offset,
                    new DoubleWordRegister(this)
                        .WithValueField(0, 31, FieldMode.Read, valueProviderCallback: _ => BUFC_buffer[i].ReadData32, name: "READDATA32"));
                registerDictionary.Add(startOffset + blockSize*i + wData32Offset,
                    new DoubleWordRegister(this)
                        .WithValueField(0, 31, FieldMode.Write, writeCallback: (_, value) => BUFC_buffer[i].WriteData32 = (uint)value, name: "WRITEDATA32"));
                registerDictionary.Add(startOffset + blockSize*i + xWrite32Offset,
                    new DoubleWordRegister(this)
                        .WithValueField(0, 31, FieldMode.Write, writeCallback: (_, value) => BUFC_buffer[i].XorWriteData32 = (uint)value, name: "XORWRITEDATA32"));
            }
        ...
        {
            // TODO
            return new DoubleWordRegisterCollection(this, new Dictionary<long, DoubleWordRegister>());
        }
        ...
        private DoubleWordRegisterCollection BuildRadioControllerRegistersCollection()
        {
            var registerDictionary = new Dictionary<long, DoubleWordRegister>
            {
                    ...
                    .WithFlag(26, FieldMode.Set, writeCallback: (_, value) =>
                        {
                            ...
                            {                                
                                ...
                                sequencer.Resume();
                                this.Log(LogLevel.Info, "Sequencer resumed, isHalted={0} VTOR={1:X} SP={2:X} PC={3:X}.", sequencer.IsHalted, sequencer.VectorTableOffset, sequencer.SP, sequencer.PC);
                            }
                        }, name: "CPUWAITDIS")
                ...
                {(long)RadioControllerRegisters.SequencerTimerValue, new DoubleWordRegister(this)
                    .WithValueField(0, 16, FieldMode.Read, valueProviderCallback: _ => { TrySyncTime(); return RAC_SeqTimerValue; })
                    .WithReservedBits(16, 16)
                },
        ...
        private DoubleWordRegisterCollection BuildProtocolTimerRegistersCollection()
        {
            var registerDictionary = new Dictionary<long, DoubleWordRegister>
            {
                ...
                {(long)ProtocolTimerRegisters.Command, new DoubleWordRegister(this)
                    ...
                    .WithFlag(8, FieldMode.Set, writeCallback: (_, value) => { if(value) PROTIMER_txRequestState = PROTIMER_TxRxRequestState.Idle; }, name: "FORCETXIDLE")
                    .WithFlag(9, FieldMode.Set, writeCallback: (_, value) => { if(value) PROTIMER_rxRequestState = PROTIMER_TxRxRequestState.Idle; }, name: "FORCERXIDLE")
                ...
                {(long)ProtocolTimerRegisters.PreCounterValue, new DoubleWordRegister(this)
                    .WithValueField(0, 16, valueProviderCallback: _ =>
                        {
                            TrySyncTime();
                            return PROTIMER_PreCounterValue;
                        }, writeCallback: (_, value) => PROTIMER_PreCounterValue = (uint)value, name: "PRECNT")
                    .WithReservedBits(16, 16)
                ...
                {(long)ProtocolTimerRegisters.BaseAndPreCounterValues, new DoubleWordRegister(this)
                    .WithValueField(0, 16, FieldMode.Read, valueProviderCallback: _ => { TrySyncTime(); return PROTIMER_PreCounterValue; }, name: "PRECNTV")
        ...
        private DoubleWordRegisterCollection BuildModulatorAndDemodulatorRegistersCollection()
        {
            var registerDictionary = new Dictionary<long, DoubleWordRegister>
            {
                {(long)ModulatorAndDemodulatorRegisters.InterruptFlags, new DoubleWordRegister(this)
                    ...
                    .WithFlag(3, out MODEM_txRampingDoneInterrupt, name: "TXRAMPDONEIF")
                ...
                {(long)ModulatorAndDemodulatorRegisters.SequencerInterruptFlags, new DoubleWordRegister(this)
                    ...
                    .WithFlag(3, out MODEM_seqTxRampingDoneInterrupt, name: "TXRAMPDONESEQIF")
        ...
        private DoubleWordRegisterCollection BuildAutomaticGainControlRegistersCollection()
        {
            var registerDictionary = new Dictionary<long, DoubleWordRegister>
            {
                ...
                {(long)AutomaticGainControlRegisters.ReceivedSignalStrengthIndicator, new DoubleWordRegister(this, 0x00008000)
                  ...
                  .WithValueField(8, 8, FieldMode.Read, valueProviderCallback: _ => (uint)AGC_RssiIntegerPart, name: "RSSIINT")
                  .WithReservedBits(16, 16)
                },
                {(long)AutomaticGainControlRegisters.FrameReceivedSignalStrengthIndicator, new DoubleWordRegister(this, 0x00008000)
                  ...
                  .WithValueField(8, 8, FieldMode.Read, valueProviderCallback: _ => (uint)AGC_FrameRssiIntegerPart, name: "FRAMERSSIINT")
                  .WithReservedBits(16, 16)
                },
        ...
        private DoubleWordRegisterCollection BuildHostMailboxRegistersCollection()
        {
            var registerDictionary = new Dictionary<long, DoubleWordRegister>
            {
                {(long)HostMailboxRegisters.MessagePointer0, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out HOSTMAILBOX_messagePointer[0], name: "MSGPTR0")
                },
                {(long)HostMailboxRegisters.MessagePointer1, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out HOSTMAILBOX_messagePointer[1], name: "MSGPTR1")
                },
                {(long)HostMailboxRegisters.MessagePointer2, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out HOSTMAILBOX_messagePointer[2], name: "MSGPTR2")
                },
                {(long)HostMailboxRegisters.MessagePointer3, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out HOSTMAILBOX_messagePointer[3], name: "MSGPTR3")
                },
                {(long)HostMailboxRegisters.InterruptFlags, new DoubleWordRegister(this)
                    .WithTaggedFlag("MBOXIF0", 0)
                    .WithTaggedFlag("MBOXIF1", 1)
                    .WithTaggedFlag("MBOXIF2", 2)
                    .WithTaggedFlag("MBOXIF3", 3)
                    .WithReservedBits(4, 28)
                    .WithChangeCallback((_, __) => UpdateInterrupts())
                },
                {(long)HostMailboxRegisters.InterruptEnable, new DoubleWordRegister(this)
                    .WithTaggedFlag("MBOXIEN0", 0)
                    .WithTaggedFlag("MBOXIEN1", 1)
                    .WithTaggedFlag("MBOXIEN2", 2)
                    .WithTaggedFlag("MBOXIEN3", 3)
                    .WithReservedBits(4, 28)
                    .WithChangeCallback((_, __) => UpdateInterrupts())
                },
            };
        ...
        private DoubleWordRegisterCollection BuildRadioMailboxRegistersCollection()
        {
            var registerDictionary = new Dictionary<long, DoubleWordRegister>
            {
                {(long)RadioMailboxRegisters.MessagePointer0, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out RFMAILBOX_messagePointer[0], name: "MSGPTR0")
                },
                {(long)RadioMailboxRegisters.MessagePointer1, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out RFMAILBOX_messagePointer[1], name: "MSGPTR1")
                },
                {(long)RadioMailboxRegisters.MessagePointer2, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out RFMAILBOX_messagePointer[2], name: "MSGPTR2")
                },
                {(long)RadioMailboxRegisters.MessagePointer3, new DoubleWordRegister(this)
                    .WithValueField(0, 31, out RFMAILBOX_messagePointer[3], name: "MSGPTR3")
                },
                ...
                {(long)RadioMailboxRegisters.InterruptEnable, new DoubleWordRegister(this)
                    .WithTaggedFlag("MBOXIEN0", 0)
                    .WithTaggedFlag("MBOXIEN1", 1)
                    .WithTaggedFlag("MBOXIEN2", 2)
                    .WithTaggedFlag("MBOXIEN3", 3)
        ...
        where T : struct, IComparable, IFormattable
        {
            ...
            if (offset >= SetRegisterOffset && offset < ClearRegisterOffset) 
            {
                ...
                {  
                    this.Log(LogLevel.Info, "SET Operation on {0}, offset=0x{1:X}, internal_offset=0x{2:X}", Enum.Format(typeof(T), internal_offset, "G"), offset, internal_offset);
                }
            } else if (offset >= ClearRegisterOffset && offset < ToggleRegisterOffset) 
            {
                ...
                if (LogRegisterAccess && !internal_read)
                {
                    this.Log(LogLevel.Info, "CLEAR Operation on {0}, offset=0x{1:X}, internal_offset=0x{2:X}", Enum.Format(typeof(T), internal_offset, "G"), offset, internal_offset);
                }
            } else if (offset >= ToggleRegisterOffset)
            {
                ...
                if (LogRegisterAccess && !internal_read)
                {
                    this.Log(LogLevel.Info, "TOGGLE Operation on {0}, offset=0x{1:X}, internal_offset=0x{2:X}", Enum.Format(typeof(T), internal_offset, "G"), offset, internal_offset);
                }
            }
        ...
        where T : struct, IComparable, IFormattable
        {
            machine.ClockSource.ExecuteInLock(delegate {
                ...
                if (offset >= SetRegisterOffset && offset < ClearRegisterOffset) 
                {
                    ...
                    {
                        this.Log(LogLevel.Info, "SET Operation on {0}, offset=0x{1:X}, internal_offset=0x{2:X}, SET_value=0x{3:X}, old_value=0x{4:X}, new_value=0x{5:X}", Enum.Format(typeof(T), internal_offset, "G"), offset, internal_offset, value, old_value, internal_value);
                    }
                } else if (offset >= ClearRegisterOffset && offset < ToggleRegisterOffset) 
                {
                    // Clear register
                    ...
                    if (LogRegisterAccess)
                    {
                        this.Log(LogLevel.Info, "CLEAR Operation on {0}, offset=0x{1:X}, internal_offset=0x{2:X}, CLEAR_value=0x{3:X}, old_value=0x{4:X}, new_value=0x{5:X}", Enum.Format(typeof(T), internal_offset, "G"), offset, internal_offset, value, old_value, internal_value);
                    }
                } else if (offset >= ToggleRegisterOffset)
                {
                    // Toggle register
                    ...
                    if (LogRegisterAccess)
                    {
                        this.Log(LogLevel.Info, "TOGGLE Operation on {0}, offset=0x{1:X}, internal_offset=0x{2:X}, TOGGLE_value=0x{3:X}, old_value=0x{4:X}, new_value=0x{5:X}", Enum.Format(typeof(T), internal_offset, "G"), offset, internal_offset, value, old_value, internal_value);
                    }
                }
        ...
        {            
            machine.ClockSource.ExecuteInLock(delegate {
                ...
                {
                    this.Log(LogLevel.Info, "{0}: IRQ RAC_RSM set", this.GetTime());
                }
                RadioControllerRadioStateMachineIRQ.Set(irq);

                irq = false;
                for(int i = 0; i < RAC_NumberOfMainCoreSequencerInterrupts; i++)
                {
                    uint mask = 1u << i;
                    if ((RAC_mainCoreSeqInterrupts.Value & mask) > 0 && (RAC_mainCoreSeqInterruptsEnable.Value & mask) > 0)
                    {
                        irq = true;
                        break;
                    }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: IRQ RAC_SEQ set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: IRQ FRC set", this.GetTime());
                }
                ...
                irq = ((MODEM_txFrameSentInterrupt.Value && MODEM_txFrameSentInterruptEnable.Value)
                       ...
                       || (MODEM_txRampingDoneInterrupt.Value && MODEM_txRampingDoneInterruptEnable.Value)
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: IRQ MODEM set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: IRQ PROTIMER set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: IRQ BUFC set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: IRQ AGC set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: SEQ_IRQ RAC set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: SEQ_IRQ FRC set", this.GetTime());
                }
                ...
                irq = ((MODEM_seqTxFrameSentInterrupt.Value && MODEM_seqTxFrameSentInterruptEnable.Value)
                       ...
                       || (MODEM_seqTxRampingDoneInterrupt.Value && MODEM_seqTxRampingDoneInterruptEnable.Value)
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: SEQ_IRQ MODEM set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                this.Log(LogLevel.Info, "{0}: SEQ_IRQ PROTIMER set", this.GetTime());
                }
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: SEQ_IRQ BUFC set", this.GetTime());
                }
                BufferControllerIRQ.Set(irq);
                ...
                if (LogInterrupts && irq)
                {
                    this.Log(LogLevel.Info, "{0}: SEQ_IRQ AGC set", this.GetTime());
                }
...
#region FRC methods
        ...
        private byte[] FRC_AssembleFrame()
        {
            ...
            for(var subframe = 0; FRC_wordCounter.Value < FRC_FrameLength; ++subframe)
            {
                ...
                {
                    this.Log(LogLevel.Error, "Read only {0} bytes from {1}", payload.Length, length);
            ...
            this.Log(LogLevel.Info, "Frame assembled: {0}", BitConverter.ToString(frame.ToArray()));
            return frame.ToArray();
        }

        private bool FRC_DisassembleFrame(byte[] frame, uint frameOffset)
        {
            bool retValue = true;
            var frameLength = 0u;
            var dynamicFrameLength = true;
            switch(FRC_dynamicFrameLengthMode.Value)
            {
                ...
                case FRC_DynamicFrameLengthMode.SingleByte:
                case FRC_DynamicFrameLengthMode.SingleByteMSB:
                    frameLength = (uint)frame[frameOffset + FRC_lengthFieldLocation.Value];
                    break;
                case FRC_DynamicFrameLengthMode.DualByteLSBFirst:
                case FRC_DynamicFrameLengthMode.DualByteMSBFirst:
                    frameLength = (uint)frame[frameOffset + FRC_lengthFieldLocation.Value + 1] << 8
                                  | (uint)frame[frameOffset + FRC_lengthFieldLocation.Value];
                    break;
                default:
                    this.Log(LogLevel.Error, "Unimplemented DFL mode.");
                    return false;
            ...
            for(var subframe = 0; FRC_wordCounter.Value < FRC_FrameLength; ++subframe)
            {
                ...
                // TODO: this might not be the right place to do this. If frame descriptors use the same BUFC, 
                // we end up saving the write offset before the last subframe.
                BUFC_buffer[descriptor.BufferIndex].UpdateWriteStartOffset();
                ...
                if(frameOffset + length > (uint)frame.Length)
                {
                    this.Log(LogLevel.Error, "frame too small, payload");
                    return false;
                }
                var payload = frame.Skip((int)frameOffset).Take((int)length);
                ...
                var pktCaptureBuff = frame.Skip((int)(frameOffset + skipCount)).Take((int)(length - skipCount));
                frameOffset += (uint)length;
                FRC_wordCounter.Value += length;
                ...
                {
                    var crc = frame.Skip((int)frameOffset).Take((int)CRC_CrcWidth).ToArray();
                    // TODO: Check CRC
                    // FRC_frameErrorInterrupt.Value |= crc check failed
                    frameOffset += (uint)crc.Length;
                ...
                if(FRC_frameErrorInterrupt.Value && FRC_rxBufferRestoreOnFrameError.Value)
                {
                    BUFC_buffer[descriptor.BufferIndex].WriteOffset = startingWriteOffset;
                }
            ...
            // RX TRAIL DATA
            {
                ...
                {
                    BUFC_buffer[descriptor.BufferIndex].WriteData = 0x40;
                }
            ...
            this.Log(LogLevel.Info, "Frame disassembled, frame descriptor now is {0}", FRC_ActiveReceiveFrameDescriptor);
...
#region RAC methods
        ...
        private void RAC_UpdateRadioStateMachine(RAC_RadioStateMachineSignal signal = RAC_RadioStateMachineSignal.None)
        {
            ...
            machine.ClockSource.ExecuteInLock(delegate {            
                ...
                {
                    this.Log(LogLevel.Info, "RAC Radio State Machine Update, signal:{0}, transition: {1}->{2}", signal, previousState, RAC_currentRadioState);
                    ...
                    switch(RAC_currentRadioState)
                    {
                        ...
                        case RAC_RadioState.Tx:                            
                            ...
                            // TODO: If we do the whole TX here without a delay, we see issues with AUTO-acking not 
                            // properly working (to be further investigated).
                            // Ideally, we want to do TX 2 stages:
                            // 1. Set some global data structure signaling that a node at a certain location
                            // is transmitting on a certain channel with a certain power (this will affect the RSSI
                            // calculations of all  nodes in range according to some model)
                            // 2. Notify all potential receivers that a transmission has started (preamble+syncword)
                            // 3. Start a timer for the duration the transmission based on the packet length and data rate
                            // 4. When timer expires, send the packet over the air and clear the global structure

                            // TODO: hardcoding 250us TX time for now. Should be based on packet length and data rate.
                            // Also see RENODE-9: an value of 100us causes a transmitter node to not go back to RX fast enough
                            // resulting in not being able to receive ACKs. This only happens when use
                            // "emulation SetGlobalSerialExecution true" in the script.
                            txTimer.Limit = 250;
                    ...
                    // If we just entered RxSearch state, we restart the Rssi sampling.
        ...
        private void RAC_PaRampingTimerHandleLimitReached()
        {
            ...
            this.Log(LogLevel.Info, "PA ramping up/down DONE at {0}", GetTime());
            // Done ramping the PA up or down, set interrupt and status flag accordingly.
            MODEM_txRampingDoneInterrupt.Value = true;
            MODEM_seqTxRampingDoneInterrupt.Value = true;
        ...
        private bool RAC_PaOutputLevelRamping
        {
            ...
            set
            {
                ...
                if (value != RAC_paOutputLevelRamping)
                {
                    ...
                    // If PA ramping is disabled, do nothing
                    if (!MODEM_rampDisable.Value)
                    {
                        paRampingTimer.Enabled = false;
                        this.Log(LogLevel.Info, "Starting PA ramping up/down at {0}", GetTime());
...
#region PROTIMER methods
            ...
            set
            {
                if (PROTIMER_LightWeightMode)
                {
                    if (value)
                    {
                        ...
                        proTimer.Limit = PROTIMER_ComputeLightWeightTimerLimit(out PROTIMER_preCounterSourcedBitmask);
                        proTimer.Value = 0;

                        // this.Log(LogLevel.Info, "PROTIMER Enabled limit={0} frequency={1}", proTimer.Limit, proTimer.Frequency);
                        // this.Log(LogLevel.Warning, "baseSource={0} baseTop={1} wrapSource={2} wrapTop={3}", (PROTIMER_BaseCounterSource)PROTIMER_baseCounterSource.Value, (uint)PROTIMER_baseCounterTop.Value,
                        //             (PROTIMER_WrapCounterSource)PROTIMER_wrapCounterSource.Value, (uint)PROTIMER_wrapCounterTop.Value);
                    }
                ...
                else
                {
                    ...
                    // TODO: we should start using the fractional part 
                    proTimer.Limit = PROTIMER_preCounterTopInteger.Value;
                    proTimer.Enabled = value;
                    for(var i = 0; i < PROTIMER_NumberOfCaptureCompareChannels; ++i)
                    {
                        PROTIMER_UpdateCompareTimer(i, false);
                    }
                }
            }
        }
        ...
        public uint PROTIMER_PreCounterValue
        {
            get
            {
                if (PROTIMER_LightWeightMode)
                {
                    // In lightweight mode we don't tick the PRECNT value, so we just return always 0.
                    return 0;
                }
                else
                {
            ...
            set
            {
                // In lightweight mode we don't tick the PRECNT so we just ignore a set.
                if (!PROTIMER_LightWeightMode)
                {
                    //TrySyncTime();
                    proTimer.Value = value;
                    for(var i = 0; i < PROTIMER_NumberOfCaptureCompareChannels; ++i)
                    {
                        PROTIMER_UpdateCompareTimer(i, false);
                    }
    ...
    public void PROTIMER_HandleChangedParams()
        {
            if (PROTIMER_LightWeightMode)
            {
                //this.Log(LogLevel.Warning, "PROTIMER_HandleChangedParams enabled={0}", PROTIMER_Enabled);
                ...
                uint bitmask;
                uint newTimerLimit = PROTIMER_ComputeLightWeightTimerLimit(out bitmask);

                
                //this.Log(LogLevel.Info, "PROTIMER Limit Updated old={0} new={1}", proTimer.Limit, newTimerLimit);
                
                // Update the sourced bitmask and restart the timer 
                PROTIMER_preCounterSourcedBitmask = bitmask;
                proTimer.Value = 0;
                proTimer.Limit = newTimerLimit;
                proTimer.Enabled = true;
            }
            else
            {
                // TODO: we should start using the fractional part in "full mode"
                if (proTimer.Limit != PROTIMER_preCounterTopInteger.Value)
                {
                    proTimer.Limit = (uint)PROTIMER_preCounterTopInteger.Value;
                }
            }
            
        }

        private uint PROTIMER_ComputeLightWeightTimerLimit(out uint sourcedBitmask)
        {
            if (!PROTIMER_LightWeightMode)
            {
                throw new Exception("PROTIMER_ComputeLightWeightTimerLimit should not be called when running PROTIMER in full mode.");
            }
            ...
            if (PROTIMER_baseCounterSource.Value == PROTIMER_BaseCounterSource.PreCounterOverflow)
            {
                ...
                sourcedBitmask |= (uint)PROTIMER_PreCountOverflowSourced.BaseCounter;
            }
            ...
            if (PROTIMER_wrapCounterSource.Value == PROTIMER_WrapCounterSource.PreCounterOverflow)
            {
                ...
                sourcedBitmask |= (uint)PROTIMER_PreCountOverflowSourced.WrapCounter;
            }

            // RENODE-19: for now if a Timeout Timer is active and sourced by PRE overflow, we switch to
            // a single tick. What we really want is to compute the number of PRE overflows to the timeout
            // or the match event.
            for(int i = 0; i < PROTIMER_NumberOfTimeoutCounters; i++)
            {
                ...
                {
                    limit = 1;
                    sourcedBitmask |= ((uint)PROTIMER_PreCountOverflowSourced.TimeoutCounter0 << i);
                }
            }
            ...
            for(int i = 0; i < PROTIMER_NumberOfCaptureCompareChannels; i++)
            {
                    ...
                    && PROTIMER_captureCompareChannel[i].mode.Value == PROTIMER_CaptureCompareMode.Compare)
                {
                        ...
                        && PROTIMER_captureCompareChannel[i].baseValue.Value > PROTIMER_baseCounterValue)
                    {
                        ...
                        sourcedBitmask |= ((uint)PROTIMER_PreCountOverflowSourced.CaptureCompareChannel0 << i);
                    }
            ...
            //this.Log(LogLevel.Warning, "PROTIMER_ComputeLightWeightTimerLimit() limit={0} mask={1:X}", limit, sourcedBitmask);
        ...
        private void PROTIMER_HandlePreCntOverflows(uint overflowCount)
        {
            if (!PROTIMER_LightWeightMode)
            {
                throw new Exception("PROTIMER_HandlePreCntOverflows should not be called when running PROTIMER in full mode.");
            }

            // this.Log(LogLevel.Warning, "PROTIMER_HandlePreCntOverflows cnt={0} mask=0x{1:X} base={2}", 
            ...
            {
                // if BASECNT is sourced by the PRECNT, we would have set the proTimer limit to either the BASECNTTOP (which is 0xFFFF max)
                // or the WRAPCNTTOP (if smaller than BASECNTTOP). Either way, it is safe to cast to ushort.
                PROTIMER_IncrementBaseCounter((ushort)overflowCount);
            }
        ...
        private void PROTIMER_HandleTimerLimitReached()
        {
            if (PROTIMER_LightWeightMode)
            {
                ...
                uint newTimerLimit = PROTIMER_ComputeLightWeightTimerLimit(out PROTIMER_preCounterSourcedBitmask);

                if (newTimerLimit != proTimer.Limit)
                {
                    //this.Log(LogLevel.Info, "Limit Updated old={0} new={1}", proTimer.Limit, newTimerLimit);

                    // Something changed, we restart the timer with the new limit
                    proTimer.Enabled = false;
                    proTimer.Value = 0;
                    proTimer.Limit = newTimerLimit;
                    proTimer.Enabled = true;
            ...
            else
            {
                ...
                PROTIMER_HandlePreCounterOverflow();
            }
        }
        ...
        private void PROTIMER_HandlePreCounterOverflow()
        {
            if (PROTIMER_LightWeightMode)
            {
                throw new Exception("PROTIMER_HandlePreCounterOverflow should not be called when running PROTIMER in lightweight mode.");
            }
        ...
        {
            // this.Log(LogLevel.Noisy, "PROTIMER_HandleBaseCounterOverflow baseValue={0} topValue={1} at {2}", 
        ...
        {
            //this.Log(LogLevel.Noisy, "PROTIMER_HandleWrapCounterOverflow wrapValue={0} at {1}", PROTIMER_WrapCounterValue, GetTime());
        ...
        private void PROTIMER_IncrementBaseCounter(ushort increment = 1)
        {
            // TODO: in full mode, increment is always = 1

            // if (PROTIMER_LightWeightMode)
            // {
            //     this.Log(LogLevel.Warning, "PROTIMER_IncrementBaseCounter value={0} incr={1}", PROTIMER_BaseCounterValue, increment);
            // }
        ...
        private void PROTIMER_IncrementWrapCounter(uint increment = 1)
        {
            ...
            // if (PROTIMER_LightWeightMode)
            // {
            //     this.Log(LogLevel.Warning, "PROTIMER_IncrementWrapCounter value={0} incr={1}", PROTIMER_WrapCounterValue, increment);
            // }
        ...
        {   
            ...
            // In lightweight mode we don't support preMatch in Compare Timers.
            // in such mode we checks that preMatch is not enabled, and if base/wrap match are enabled, we 
            // recalculate the protimer limit.
            if (PROTIMER_LightWeightMode)
            {
            ...
            {
                // Channel is disabled or set in capture mode or PRE match is disabled, nothing to do here 
                // (BASE and WRAP matches are handled in the respective increment routines).
                if (!PROTIMER_captureCompareChannel[index].enable.Value 
                    || PROTIMER_captureCompareChannel[index].mode.Value != PROTIMER_CaptureCompareMode.Compare
                    || !PROTIMER_captureCompareChannel[index].preMatchEnable.Value)
                {
                ...
                if(PROTIMER_captureCompareChannel[index].preValue.Value < proTimer.Value && PROTIMER_captureCompareChannel[index].preValue.Value < proTimer.Limit)
                {
                    PROTIMER_captureCompareChannel[index].timer.Limit = PROTIMER_captureCompareChannel[index].preValue.Value;
                    PROTIMER_captureCompareChannel[index].timer.Value = proTimer.Value;
                    PROTIMER_captureCompareChannel[index].timer.Enabled = true;
                }
            }
        }
        ...
        {   
            ...
            switch(PROTIMER_listenBeforeTalkState)
            {
                ...
                {
                    // If we get here is because CCA was successful, otherwise we would have retarted the backoff or failed LBT
        ...
        private void PROTIMER_ListenBeforeTalkCcaCompleted()
        {
            bool ccaSuccess = (AGC_RssiIntegerPart < (int)AGC_ccaThreshold.Value);
        ...
        {
            if(ev < PROTIMER_Event.PreCounterOverflow || ev > PROTIMER_Event.TimeoutCounter0MatchListenBeforeTalk)
            {
            ...
            switch(PROTIMER_rxRequestState)
            {
                ...
                {
                    if (PROTIMER_rxSetEvent2.Value == ev)
                    {
...
#region CRC methods
        ...
        {
            this.Log(LogLevel.Warning, "CRC mocked with 0x0 bytes.");
...
#region AGC methods
        ...
        private uint AGC_RssiPeriodUs
        {
            get
            {
                ...
                var symbolTimeUs = AGC_OQPSK250KbpsPhyBaudPeriodUs;
                var rssiPeriod = AGC_ccaRssiPeriodEnable.Value ? AGC_ccaRssiPeriod.Value : AGC_rssiMeasurePeriod.Value;
                // If SUBPERIOD is set, the subperiod is defined by SUBINT + SUBNUM/SUBDEM. 
                // Otherwise subperiod is equal to 1 baud.
                // TODO: for now we consider only SUBINT.
                var subPeriod = AGC_subPeriod.Value ? AGC_subPeriodInteger.Value : 1;
                // RSSI measure period is defined as 2^RSSIPERIOD
                return (uint)((1 << (int)rssiPeriod))*(uint)subPeriod*(uint)symbolTimeUs;
            }
        }

        private void AGC_UpdateRssi()
        {
            // TODO: for now RSSI is hardcoded. It should be based on the "over-the-air" conditions. 
            // That will be possible once TX/RX are no longer instantanous operations and we will implement and interference queue.
            AGC_RssiIntegerPart = AGC_RssiIntegerPartHardcodedValue;
            ...
            if (AGC_RssiIntegerPart < (int)AGC_ccaThreshold.Value)
            {
            ...
            if (AGC_RssiIntegerPart > (int)AGC_rssiHighThreshold.Value)
            {
                AGC_rssiHighInterrupt.Value = true;
                AGC_seqRssiHighInterrupt.Value = true;
            }
            else if (AGC_RssiIntegerPart < (int)AGC_rssiLowThreshold.Value)
            {
        ...
        private void AGC_RssiStartCommand(bool fromProtimer = false)
        {
...
#region RAC fields
        ...
        private const uint RAC_PowerAmplifierRampingTimeUs = 10;
        private bool RAC_paOutputLevelRamping = false;
        private const uint RAC_NumberOfMainCoreSequencerInterrupts = 8;
        ...
        private IValueRegisterField[] RAC_seqStorage;
        private const uint RAC_NumberOfScratchRegisters = 8;
        private IValueRegisterField[] RAC_scratch;
...
#region PROTIMER fields
        private readonly bool PROTIMER_LightWeightMode = true;
...
#region MODEM fields
        ...
        private uint MODEM_SyncWord0 => (uint)MODEM_syncWord0.Value & (uint)((1 << (byte)MODEM_SyncWordLength) - 1);
        private uint MODEM_SyncWord1 => (uint)MODEM_syncWord1.Value & (uint)((1 << (byte)MODEM_SyncWordLength) - 1);
        ...
        private IFlagRegisterField MODEM_txRampingDoneInterrupt;
        ...
        private IFlagRegisterField MODEM_seqTxRampingDoneInterrupt;
...
#region AGC fields
        // TODO
        // TX/RX are currently instanteous operations.
        // When we introduce the "over the air packet queue" we will use that
        // together with the geographic position of this node and each transmitter
        // node to model a more accurate RSSI value.
        // For now AGC integer part is always -100.
        private const sbyte AGC_RssiIntegerPartHardcodedValue = -100;
        // TODO
        // Similarly to the above comment, we also hardcode the Frame RSSI to a reasonable value (10).
        private const sbyte AGC_FrameRssiIntegerPartHardcodedValue = 10;
        private const uint AGC_OQPSK250KbpsPhyBaudPeriodUs = 1U;
...
private IValueRegisterField[] HOSTMAILBOX_messagePointer = new IValueRegisterField[4];
#endregion


#region Radio Mailbox Fields
private IValueRegisterField[] RFMAILBOX_messagePointer = new IValueRegisterField[4];
#endregion
...
#region Register enums
        ...
        private enum SynthesizerRegisters : long
        {
            ...
            FrequencySynthesizerChannelControl                              = 0x0040,
            ...
            FrequencySynthesizerChannelControl_Set                          = 0x1040,
            ...
            FrequencySynthesizerChannelControl_Clr                          = 0x2040,
            ...
            FrequencySynthesizerChannelControl_Tgl                          = 0x3040,
        ...
        private class PROTIMER_TimeoutCounter
        {
            ...
            {
                // TODO: handle evtCount > 1
                if ((running.Value || synchronizing.Value) && evtCount > 1)
                {
                    parent.Log(LogLevel.Error, "TOUT{0} Update() passed an evtCount > 1 ({1})", index, evtCount);
                }
        ...
        private class BUFC_Buffer
        {
            ...
            public uint ReadData
            {
                get
                {
                    if (BytesNumber == 0)
                    {
                        ...
                        parent.Log(LogLevel.Info, "Buffer #{0}: Reading underflow Size={1} writeOffset={2} readOffset={3}", 
                    ...
                    parent.Log(LogLevel.Info, "Buffer #{0}: Reading value 0x{1:X} at ram's offset 0x{2:X}, oldOffset={3} newOffset{4}", 
                               index, value, offset, ReadOffset, newOffset);
                    ReadOffset = newOffset;

                    if (underflow.Value || seqUnderflow.Value)
                    {
                        parent.UpdateInterrupts();
                    }
            ...
            public uint ReadData32
            {
                get
                {
                    ...
                    if (BytesNumber < 4)
                    {
                        ...
                        parent.Log(LogLevel.Info, "Buffer #{0} underflow: Reading value (32bit) (partial {1} bytes) 0x{2:X} at ram's offset 0x{2:X}", 
                    ...
                    else
                    {
                        ...
                        parent.Log(LogLevel.Info, "Buffer #{0}: Reading value (32bit) 0x{1:X} at ram's offset 0x{2:X}, oldOffset={3} newOffset={4}", 
                                   index, value, offset, ReadOffset, newOffset);
                        ReadOffset = newOffset;
                    }
            ...
            public uint WriteData
            {
                set
                {
                    if (BytesNumber == Size)
                    {
                        ...
                        parent.Log(LogLevel.Info, "Buffer #{0}: Writing overflow Size={1} writeOffset={2} readOffset={3}", 
                                                   index, Size, WriteOffset, ReadOffset);
                        return;
                    }
                    ...
                    parent.Log(LogLevel.Info, "Buffer #{0}: Writing value 0x{1:X} at ram's offset 0x{2:X}, oldOffset={3} newOffset={4}, readOffset={5}", 
                                               index, value, offset, WriteOffset, newOffset, ReadOffset);
                    WriteOffset = newOffset;

                    if (overflow.Value || seqOverflow.Value)
                    {
                        parent.UpdateInterrupts();
                    }
            ...
            public uint WriteData32
            {
                set
                {
                    if (BytesNumber > Size - 4)
                    {
                        overflow.Value = true;
                        seqOverflow.Value = true;
                        parent.Log(LogLevel.Info, "Buffer #{0}: Writing overflow (32bit) Size={1} writeOffset={2} readOffset={3}", 
                                                   index, Size, WriteOffset, ReadOffset);
                        return;
                    }
                    ...
                    parent.Log(LogLevel.Info, "Buffer #{0}: Writing value (32bit) 0x{1:X} at ram's offset 0x{2:X}, oldOffset={3} newOffset={4} readOffset={5}", 
                                               index, value, offset, WriteOffset, newOffset, ReadOffset);
                    WriteOffset = newOffset;
            ...
            public uint XorWriteData
            {
                set
                {
                    if (BytesNumber == Size)
                    {
                        overflow.Value = true;
                        seqOverflow.Value = true;
                        parent.Log(LogLevel.Info, "Buffer #{0}: XOR Writing overflow Size={1} writeOffset={2} readOffset={3}", 
                                                   index, Size, WriteOffset, ReadOffset);
                        return;
                    }
                    ...
                    var newOffset = (WriteOffset + 1) % (2* Size);
                    parent.Log(LogLevel.Info, "Buffer #{0}: XOR Writing value 0x{1:X} (0x{2:X} ^ 0x{3:X}) at ram's offset 0x{4:X}, oldOffset={5} newOffset={6} readOffset={7}", 
            ...
            public uint XorWriteData32
            {
                set
                {
                    if (BytesNumber > (Size - 4))
                    {
                        overflow.Value = true;
                        seqOverflow.Value = true;
                        parent.Log(LogLevel.Info, "Buffer #{0}: XOR Writing overflow (32bit) Size={1} writeOffset={2} readOffset={3}", 
                                                   index, Size, WriteOffset, ReadOffset);
                        return;
                    }
                    ...
                    var newOffset = (WriteOffset + 4) % (2 * Size);
                    parent.Log(LogLevel.Info, "Buffer #{0}: XOR Writing value (32bit) 0x{1:X} (0x{2:X} ^ 0x{3:X}) at ram's offset 0x{4:X}, oldOffset={5} newOffset={6} readOffset={7}", 
                                               index, newData, oldData, value, offset, WriteOffset, newOffset, ReadOffset);
                    WriteOffset = newOffset;
            ...
            {
                // TODO: the PREFETCH command issued by the sequncer while the buffer is empty is causing the underflow interrupt to be set
                // and stay set. We need to figure out when this should be cleared before re-enabling prefetching. 
                // It is also sort of a no-op the way this is implemented.

                /*
                if (BytesNumber == 0)
                {
                    parent.Log(LogLevel.Error, "Buffer #{0}: Prefetch while BytesNumber==0, underflow", index);
            ...
            {
                parent.Log(LogLevel.Info, "Buffer #{0}: Saving write offset {1}", index, writeOffset);
                writeStartOffset.Value = WriteOffset;
            }

            public void RestoreWriteOffset()
            {
                parent.Log(LogLevel.Info, "Buffer #{0}: Restoring write offset {1}", index, writeStartOffset.Value);
            ...
            {
                parent.Log(LogLevel.Info, "Buffer #{0}: Updating Threshold flag", index);
                if (ThresholdFlag)
                {
                    parent.Log(LogLevel.Info, "Buffer #{0}: Threshold flag set, bytes {1} threshold {2}", index, BytesNumber, threshold.Value);
