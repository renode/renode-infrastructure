...
namespace Antmicro.Renode.Peripherals.Analog
{
   ...
   {
      public const int NUM_CHANNELS = 19;

      public STM32_ADC(Machine machine) : base(machine)
      {
         channels = Enumerable.Range(0, NUM_CHANNELS).Select(x => new ADCChannel(this, x)).ToArray();

         DefineRegisters();

         // Sampling time fixed
         samplingTimer = new LimitTimer(
               machine.ClockSource, 1000000, this, String.Format("samplingClock"),
               limit: 100,
               eventEnabled: true,
      ...
      private void DefineRegisters()
      {
         ...
         Registers.Control2.Define(this, name: "Control2")
            .WithFlag(0, out adcOn,
                  ...
                  writeCallback: (oldVal, val) => {if (oldVal != val) {EnableADC(val);}})
            ...
            .WithFlag(30,
                  ...
                  writeCallback: (_, value) => StartConversion(value),
         ...
         .WithReadCallback((_,__) => ReadData());
      }
      ...
      private void EnableADC(bool on)
      {
         if (on)
         {
      ...
      private void StartConversion(bool start)
      {
         if (adcOn.Value)
         {
            if (start)
      ...
      private void OnConversionFinished()
      {
         ...
         bool scanModeActive = scanMode.Value && currentChannelIdx < regularSequenceLen - 1;
         bool scanModeFinished = scanMode.Value && currentChannelIdx == regularSequenceLen - 1;
         ...
         {
            this.Log(LogLevel.Info, "OnConversionFinished: Set IRQ");
            IRQ.Set(true);
         }
      }
