...
namespace Antmicro.Renode.Peripherals.Bus
{
        ...
        IMultibyteWritePeripheral
    {
        ...
        ICPU GetCurrentCPU();
        IEnumerable<IBusRegistered<IBusPeripheral>> GetPeripheralsForContext(ICPU context);
            ...
            bus.MoveRegistrationWithinContext(peripheral, newAddress, cpu,
                selector: busRegisteredEnumerable =>
                {
                    return busRegisteredEnumerable.Where(
                            busRegistered => (busRegistered.RegistrationPoint as BusMultiRegistration).ConnectionRegionName == regionName
                        ).Single();
                }
            );
        }
