...
namespace Antmicro.Renode.Core
{
    public class Machine : IEmulationElement, IDisposable
    {
        ...
        public void HandleTimeDomainEvent<T>(Action<T> handler, T handlerArgument, TimeStamp eventTime, Action postAction = null)
        {
            ...
            if(emulation.NonDeterministicMode)
            {
                CheckRecorder();
        ...
        {
            if(EmulationManager.Instance.CurrentEmulation.NonDeterministicMode)
            {
                throw new RecoverableException("Recording events is not available in the non-deterministic mode");
            }
        ...
        public void PlayFrom(string fileName)
        {
            if(EmulationManager.Instance.CurrentEmulation.NonDeterministicMode)
            {
                throw new RecoverableException("Replying events is not available in the non-deterministic mode");
            }
        ...
        private void CheckRecorder()
        {
            if(recorder != null)
            {
                this.Log(LogLevel.Error, "Detected existing event recorder attached to the machine - it won't work in the non-deterministic mode");
            }
        }
