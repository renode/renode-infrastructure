...
namespace Antmicro.Renode.UserInterface.Commands
{
    public class IncludeFileCommand : Command
    {
        ...
        {
            using (var progress = EmulationManager.Instance.ProgressMonitor.Start("Including script(s): " + String.Join(' ', paths.Select(nameof => nameof.ToString()).ToArray()) ) )
            {
                bool result = false;
                ...
                if (ext != ".cs")
                {
                    if (paths.Length > 1)
                    {
                        writer.WriteError($"Include of multiple files only for *.cs!");
                        return false;
                    }
                }
