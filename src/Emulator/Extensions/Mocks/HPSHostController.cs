...
namespace Antmicro.Renode.Extensions.Mocks
{
    ...
    public class HPSHostController : IHostMachineElement, IConnectable<II2CPeripheral>
    {
        ...
        private string[,] FormatSystemCommands(byte[] data)
        {
            if(data.Length != 2)
            {
                throw new RecoverableException(string.Format("Received {0} bytes of data (expected 2) from System Commands register.", data.Length));
            }
            var table = new Table().AddRow("Value", "Bit", "Name", "Description");
            table.AddRow((data[0] & 0x80) >> 7 == 1 ? "1" : "0", "15", "ERASE_STAGE0", "Erase stage0 (one_time_init)");
            table.AddRow((data[0] & 0x40) >> 6 == 1 ? "1" : "0", "14", "WRITE_TEST_DATA", "Write SPI flash test data");
            ...
            table.AddRow((data[1] & 0x10) >> 4 == 1 ? "1" : "0", "4", "ERASE_SPIFLASH", "Erase SPI flash (FPGA bitstream and application)");
            table.AddRow((data[1] & 0x8) >> 3 == 1 ? "1" : "0", "3", "ERASE_STAGE1", "Erase RW region of MCU flash (stage1)");
            table.AddRow((data[1] & 0x4) >> 2 == 1 ? "1" : "0", "2", "LAUNCHAPP", "Launch application (from stage1)");
            table.AddRow((data[1] & 0x2) >> 1 == 1 ? "1" : "0", "1", "LAUNCH1", "Launch stage1 (from stage0)");
            table.AddRow((data[1] & 0x1) == 1 ? "1" : "0", "0", "RESET", "Reset module to stage0");
            return table.ToArray();
        }
